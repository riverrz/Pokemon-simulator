...
let attackdata = fs.readFileSync('../data/pokedex/moves.json');
...
var damage_calc = function (){
  ...
  if (random() < 1){
    ...
    return  ((((2*level/5+2)*Move_Power*AtkStat/DefStat)/50)+2)*Modifier +"%";
  }
}

var effect = function() {
  if (moves[attack].effect.chance || moves[attack].effect.stat || moves[attack].effect.stages ){
    let chance = moves[attack].effect.chance;
    return (" Chance: " + moves[attack].effect.chance +" Stat:" + moves[attack].effect.stat + " Stages:" + moves[attack].effect.stages);
  } else {
...
console.log(pokemon[attacker].name + " Used " + moves[attack].name + " attack! ");
...
console.log(pokemon[target].name + " got hit by " + moves[attack].name + "! attack");
